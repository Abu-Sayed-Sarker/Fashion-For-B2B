...
const SelectField = ({ label, required, value, onChange, options, placeholder, helperText }) => (
  <div>
...
export default function StapesTow({ goToNextStep, goToPreviousStep }) {
  ...
  const mandatoryMeasurements = [
    { id: 1, pom: "Chest", instruction: "Measure 1\" below armhole, straight across chest", locked: true },
    { id: 2, pom: "Body Length", instruction: "Measure from HPS (high point shoulder) to bottom hem", locked: true },
    { id: 3, pom: "Sleeve Length", instruction: "Measure from shoulder seam to sleeve hem", locked: true },
    { id: 4, pom: "Shoulder Width", instruction: "Measure from shoulder seam to shoulder seam across back", locked: true },
    { id: 5, pom: "Armhole", instruction: "Measure armhole circumference from shoulder seam", locked: true },
    { id: 6, pom: "Hem Width", instruction: "Measure bottom hem width edge to edge when laid flat", locked: true },
    { id: 7, pom: "Neck Opening", instruction: "Measure neckline opening circumference", locked: true },
    { id: 8, pom: "Waist", instruction: "Measure waist circumference", locked: true },
  ];
  ...
  const handleSubmit = () => {
    ...
    if (!requiredMeasurements.every(m => m.pom && m.measurement_value && m.unit && m.tolerance)) {
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-700 text-sm">
              Required measurements are automatically loaded based on garment type. You can add additional custom measurements using the button below.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {measurements.map((measurement, index) => (
                  <tr key={measurement.id} className={measurement.locked ? "bg-yellow-50" : "bg-white"}>
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        onChange={(e) => updateMeasurement(measurement.id, "pom", e.target.value)}
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        onChange={(e) => updateMeasurement(measurement.id, "measurement_value", e.target.value)}
                        placeholder="0.2"
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm text-gray-700">{defaultUnit || "cm"}</span>
                    </td>
                    <td className="px-4 py-3">
                      <input
                        type="text"
                        value={measurement.instruction}
                        onChange={(e) => updateMeasurement(measurement.id, "instruction", e.target.value)}
          ...
          <div className="border-t border-gray-200 p-4 flex items-center justify-between bg-gray-50">
            ...
            <div className="text-sm text-gray-600">
              <span className="font-medium">{totalCount}</span> measurements total • <span className="font-medium">{requiredCount}</span> required
            </div>
          </div>
        </div>
